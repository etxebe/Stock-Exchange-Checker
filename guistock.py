...
class GUI:
    def __init__(self, master):
        ...
        editmenu.add_command(label="Refresh")
        ...
        self.headline = Label(master, bg="#b3ccff", text="GPW", font="Verdana 10 bold")
        ...
        for i in range(len(stockchecker.MY_COMPANIES)):
            if stockchecker.CHANGE[i][0] == '-':
                ...
                photo_file = "stock_down.png"
            column = 0
            self.company = Label(master, bg="#b3ccff", text=stockchecker.COMPANY[i], font="Verdana 10 bold", fg=color)
            self.company.grid(row=row, column=column)
            column += 1
            self.price = Label(master, bg="#b3ccff", text=stockchecker.PRICE[i], font="Verdana 10 bold", fg=color)
            self.price.grid(row=row, column=column)
            column += 1
            self.change = Label(master, bg="#b3ccff", text=stockchecker.CHANGE[i], font="Verdana 10 bold", fg=color)
            self.change.grid(row=row, column=column)
            column += 1
            photo = PhotoImage(file=photo_file)
            self.photo_label = Label(master, image=photo)
            self.photo_label.photo = photo
            self.photo_label.grid(row=row, column=column)
            column += 1
            self.percentage = Label(master, bg="#b3ccff", text=stockchecker.PERCENTAGE_CHANGE[i], font="Verdana 10 bold", fg=color)
            self.percentage.grid(row=row, column=column)
            column += 1
            self.date = Label(master, bg="#b3ccff", text=stockchecker.DATE[i], font="Verdana 10 bold", fg=color)
